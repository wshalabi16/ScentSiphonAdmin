    ...
    return(
        <Layout>
            ...
            <form onSubmit={saveCategory} className="flex gap-1">
                <input className="mb-0" type="text" placeholder={'Category Name'} onChange={event => setName(event.target.value)} value={name}/>
                ...
                <tbody>
                    {categories.length > 0 && categories.map(category => (
                        <tr key={category._id}>
                            ...
                            <button onClick={() => deleteCategory(category)} className="btn-red mr-1">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
