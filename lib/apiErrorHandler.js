...
/**
 * Sanitize string values to prevent log injection attacks
  ...
  return async (request) => {
    ...
    try {
      ...
      console.info(JSON.stringify({
        ...
        path: sanitizeLogValue(new URL(request.url).pathname),
        userId: sanitizeLogValue(userEmail),
      ...
      console.error(JSON.stringify({
        ...
        ip: sanitizeLogValue(ip),
        duration: `${duration}ms`,
        errorType: sanitizeLogValue(error.name),
        errorMessage: sanitizeLogValue(error.message),
        errorStack: process.env.NODE_ENV === 'development' ? sanitizeLogValue(error.stack) : undefined,
      }));
